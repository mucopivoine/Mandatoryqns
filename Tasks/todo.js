...
class Task {
  ...
  class ToDoList {
    ...
    markTaskAsCompleted(details) {
      ...
      if (task) {
        ...
        console.log(`Task with "${details}" not included.`);
      }
    }
    removeTask(details) {
      ...
      if (taskIndex !== -1) {
       const removeTask= this.tasks.splice(taskIndex, 1)[0];
        console.log(`Task "${removeTask.type}" removed from the list.`);
      } else {
        console.log(`Task  with details"${details}" not included.`);
      }
    }
  }
  ...
  toDoList.addTask("wash dishes", "simple");
